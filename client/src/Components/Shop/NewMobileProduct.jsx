...
const NewMobileProduct = ({
    ShopPretezelProducts,
    ShopOlivesProducts,
    ShopChocoPretzelProducts,
    ShopSnacksBundleProducts,
    MerchProducts,
    BestSellerProducts,
}) => {
    const shopProductsdata = ShopPretezelProducts
        ? ShopPretezelProducts
        : ShopOlivesProducts
        ? ShopOlivesProducts
        : ShopChocoPretzelProducts
    ...
    // color function
    const olivColor =
        ShopOlivesProducts || ShopSnacksBundleProducts
            ? { color: "#412f59", borderColor: "#412f59" }
            : { color: "#4c4294", borderColor: "#4c4294" };
            ...
            >
                {shopProductsdata.map((items) => {
                    return (
                        <SwiperSlide>
                            <div
                                className="ShopProducts col-sm-2 container g-0 "
                                key={items.id}
                            >
                                <div className="ShopProducts-content text-center">
                                    <div className="ShopProducts-imge d-flex">
                                        <img
                                            className="ShopProduct-img1"
                                            src={items.productimge1}
                                            alt=""
                                        />
                                        <br />
                                        <img
                                            className="ShopProduct-imghover"
                                            src={items.imgehover}
                                            alt=""
                                        />
                                    </div>
                                    <div
                                        className="ShopProductsheader"
                                        style={olivColor}
                                    >
                                        {items.title}
                                    </div>
                                    <div
                                        className="ShopProducts-size text-uppercase fw-bold"
                                        style={olivColor}
                                    >
                                        {items.sizeinfo}
                                    </div>
                                    <div
                                        className="ShopProductsPrice fw-bold fs-5"
                                        style={olivColor}
                                    >
                                        <span>£</span>
                                    ...
                                    {items?.reviews !== "" ? (
                                        <div className="ShopProduct-starreview">
                                            <span style={olivColor}>
                                                <span
                                                    className="jdgm-star jdgm--on bold"
                                                    style={olivColor}
                                                >
                                                    <BsFillStarFill />
                                                </span>{" "}
